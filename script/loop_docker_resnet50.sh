...
for i in $(seq 0 $INDEX); do
  echo "ai card$i"
  if [ $i -lt $INDEX ]; then
    echo "./docker_resnet50.sh $i $CASE > resnet_card$i.$CASE.log 2>&1 &"
          ./docker_resnet50.sh $i $CASE > resnet_card$i.$CASE.log 2>&1 &
  else
    echo "./docker_resnet50.sh $i $CASE > resnet_card$i.$CASE.log last"
          ./docker_resnet50.sh $i $CASE > resnet_card$i.$CASE.log 2>&1
  fi
  sleep 1
done
